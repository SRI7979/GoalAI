...
export default function FlashcardView({ task, goal, knowledge, domain = null, onClose, onComplete }) {
  ...
  useEffect(() => {
    async function load() {
      const cacheKey = `pathai.flashcard.v2::${domain || 'domainless'}::${task.id || task.title}`
      try {
    ...
    load()
  }, [task.id, task.title, goal, knowledge, task._concept, task.description, task.action, task.outcome, learningContract, domain])
  ...
  return (
    <>
      ...
      <AIAssistant concept={task._concept || task.title} goal={goal} domain={domain} knowledge={knowledge} context={`Flashcard ${current + 1}/${cards.length}: ${card?.front || ''}`} />
    </>
  )
}
